...
t_position findNearestReachablePoint(t_map map) {
    ...
    for (int i = 0; i < map.y_max; i++) {
        for (int j = 0; j < map.x_max; j++) {
            // Skip crevasses and base station
            if (map.soils[i][j] == CREVASSE || map.soils[i][j] == BASE_STATION) {
                continue;
            }

            // Calculate Manhattan distance to base
            int distance = abs(j - basePos.x) + abs(i - basePos.y);
    ...
    if (nearestPoint.x != -1 && nearestPoint.y != -1) {
        printf("Nearest reachable point to base is at: [%d, %d] with distance: %d\n", 
               nearestPoint.x, nearestPoint.y, minDistance);
    } else {
